...
const map: Record<string, { label: string; cls: string; dot?: boolean; dotCls?: string; animated?: boolean; icon?: string }> = {
  pending:        { label: '대기',         cls: 'bg-gray-100 text-gray-600',     icon: 'clock' },
  ocr_running:    { label: 'OCR 처리중',   cls: 'bg-blue-50 text-blue-600',      dot: true, dotCls: 'bg-blue-500', animated: true },
  ocr_done:       { label: 'OCR 완료',     cls: 'bg-blue-50 text-blue-600',      dot: true, dotCls: 'bg-blue-500' },
  ingest_running: { label: 'AI 처리중',    cls: 'bg-indigo-50 text-indigo-600',  dot: true, dotCls: 'bg-indigo-500', animated: true },
  ingest_done:    { label: 'Ingest 완료',  cls: 'bg-indigo-50 text-indigo-600',  dot: true, dotCls: 'bg-indigo-500' },
  done:           { label: '완료',         cls: 'bg-green-50 text-green-700',    icon: 'check' },
  failed:         { label: '실패',         cls: 'bg-red-50 text-red-600',        icon: 'fail' },
  running:        { label: '실행중',       cls: 'bg-blue-50 text-blue-600',      dot: true, dotCls: 'bg-blue-500', animated: true },
